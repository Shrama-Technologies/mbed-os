...
MBED_WEAK void ticker_set_handler(const ticker_data_t *const data, ticker_event_handler handler) {
    data->interface->init();

    data->queue->event_handler = handler;
}

MBED_WEAK void ticker_irq_handler(const ticker_data_t *const data) {
...
MBED_WEAK void ticker_insert_event(const ticker_data_t *const data, ticker_event_t *obj, timestamp_t timestamp, uint32_t id) {
...
MBED_WEAK void ticker_remove_event(const ticker_data_t *const data, ticker_event_t *obj) {
    __disable_irq();
...
MBED_WEAK timestamp_t ticker_read(const ticker_data_t *const data)
{
    return data->interface->read();
}

MBED_WEAK int ticker_get_next_timestamp(const ticker_data_t *const data, timestamp_t *timestamp)
{
    int ret = 0;
