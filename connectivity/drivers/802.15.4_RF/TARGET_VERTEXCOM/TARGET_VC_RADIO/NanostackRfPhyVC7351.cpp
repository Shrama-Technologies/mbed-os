 ...
 */
#include "NanostackRfPhyVC7351.h"
...
static NanostackRfPhyVC7351 *rf = NULL;
//
...
NanostackRfPhyVC7351::NanostackRfPhyVC7351() : NanostackRfPhy()
{
    // Do nothing
}

NanostackRfPhyVC7351::~NanostackRfPhyVC7351()
{
    rf_unregister();
}

int8_t NanostackRfPhyVC7351::rf_register()
{
    ...
    if (rf != NULL) {
        rf_if_unlock();
        error("Multiple registrations of NanostackRfPhyVC7351 not supported");
        return -1;
    }
...
void NanostackRfPhyVC7351::rf_unregister()
{
    rf_if_lock();
...
void NanostackRfPhyVC7351::get_mac_address(uint8_t *mac)
{
    rf_if_lock();
...
void NanostackRfPhyVC7351::set_mac_address(uint8_t *mac)
{
    rf_if_lock();

    if (NULL != rf) {
        error("NanostackRfPhyVC7351 cannot change mac address when running");
        rf_if_unlock();
        return;
    }
...
//
//uint32_t NanostackRfPhyVC7351::get_driver_version()
//{
...
//                *
//                * For VC7351xg1x parts, frame errors include violations of variable length
...
{
    static NanostackRfPhyVC7351 rf_phy;
    return rf_phy;
}
